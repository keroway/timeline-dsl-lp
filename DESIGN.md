...
| ページ | ヘッダー実装 | 理由 |
| --- | --- | --- |
| LP (`/`) / Playground (`/playground/`) / Changelog (`/changelog/`) | `SiteHeader.astro` を直接利用 | 独立ページであり、ナビゲーション・a11y メニューをフルコントロールする必要があるため |
| Docs (`/docs/...`) | Starlight 標準 `Header` ＋ `SocialIcons` override | Starlight が提供するサイドバートリガー・検索（Pagefind）・モバイルメニューを壊さないために、Starlight の `components.Header` は維持する |

Docs ヘッダーの a11y メニューは `DocsA11yMenu.astro`（`components.SocialIcons` として登録）で提供します。GitHub ソーシャルリンクも同コンポーネント内に含めているため、`astro.config.mjs` の `social` 配列は表示先を `DocsA11yMenu.astro` が担います。
...
#### 将来 SiteHeader で Docs ヘッダーを統一する場合の前提条件

Starlight の `components.Header` を `SiteHeader.astro` に差し替える（方針 A）には以下が必要です。検討時はこのリストを参照してください。

- Starlight の `<SiteTitle />`（ロゴ + サイト名）、`<Search />`（Pagefind）、`<MobileMenuToggle />`、`<ThemeSelect />` を `SiteHeader.astro` 内で自前レンダリングする
- Starlight の `--sl-nav-height` / `--sl-sidebar-width` 等の CSS 変数を尊重したレイアウトに書き直す
- モバイル時のサイドバー開閉トリガーを維持する

現状はこの前提条件を満たす改修コストが便益を上回るため、**方針 B（`SocialIcons` override）を継続**します。
...
- `site/src/components/SiteHeader.astro` — LP / Playground / Changelog のヘッダーと a11y メニューの実装
