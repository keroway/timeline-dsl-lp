...
- **Calm density.** 静的なヒーロー → 機能 → ワークフロー → ユースケース → Examples → Why → インストール → クロージング CTA という積層は維持する。1 画面で過剰に語らず、各セクションが 1 つのメッセージだけを持つ。install band は「導入手順」に専念し、行動喚起はページ末尾のクロージング CTA が担う（#434）。
...
### Examples section（#433）

- Usecases と Why の間に配置。`gallery-samples.json` から用途の異なる 3 件（歴史 / 創作 / OSS）を選び、`TimelineEmbed.astro`（#428 と同じ `renderSvgFromSource` 経路）でビルド時に静的 SVG として埋め込む。
- クライアント WASM 起動は LP に持ち込まない（Lighthouse の LCP / TBT を守る）。各カードは `TimelineEmbed` 組み込みの「Playground で編集する」導線をそのまま使う。セクション末尾に「すべてのサンプルを見る（Gallery）」への導線を置く。
- lane の dark / high-contrast 追従は WASM 出力の `class="tdsl-root"` 経由で自動適用される（§8 lane bridge）。追加の CSS 変更は不要。
