...
- インストールコマンドは現状 `brew install keroway/tap/timeline-dsl` の 1 行。Copy ボタンには `data-copy-target` で対応 ID を指定し、フォールバック（`execCommand`）も含めて実装済み。新規導入チャネルが増えたときは別バンドに分けるか、タブで切り替える設計を先に決める。
...
- LP 内の `<pre><code>` は現状ハイライトなし（プレーンテキスト）。Timeline DSL 用のシンタックスハイライト導入は #53 で別途検討。

### Docs code block / Copy button

- Docs（`/docs/...`）配下のコードブロックは Starlight 標準の Expressive Code が描画する。LP 側の `<pre><code>` とは別系統。
...
- アイコン化検討の経緯は #62。現状の EC 標準が既に icon-only であり、追加カスタマイズの利点が小さいため据え置きとした。
...
- transform / opacity 以外のプロパティをアニメートしない（パフォーマンスとちらつきの観点）。
