...
> **English summary**: This document records the design decisions for the `timeline-dsl-lp` site (LP + Docs). Three core principles guide every decision: **Source as truth** (the site reflects Timeline DSL's text-driven philosophy — code, commands, and rendered output appear side-by-side); **Editor first** (primary audience is developers and educators, so readability and CLI output fidelity take priority over decoration); **Calm density** (sections stack in a fixed order — hero → features → workflow → use cases → why → install → closing CTA — with one message per section). Colors are defined as CSS custom properties in `site/src/styles/global.css`; always reuse existing tokens before adding new ones. Typography relies on the Starlight system font stack with `--font-code` for monospace. When in doubt, choose honesty over spectacle and code legibility over visual flourish.
...
- **Calm density.** 静的なヒーロー → 機能 → ワークフロー → ユースケース → Why → インストール → クロージング CTA という積層は維持する。1 画面で過剰に語らず、各セクションが 1 つのメッセージだけを持つ。install band は「導入手順」に専念し、行動喚起はページ末尾のクロージング CTA が担う（#434）。
...
`localStorage` キー `tdsl-a11y-settings` と `<html>` の `data-a11y-*` 属性の仕様は `SiteHeader.astro` と共通です。Docs で保存した設定は LP / Playground / Changelog でもそのまま引き継がれます（同じキーを読み書きするため）。テーマは #431 で `starlight-theme` + `<html data-theme="light|dark">` を LP 側も採用し、Docs ⇄ LP の往復で light / dark / auto の選択が保持される。
...
- LP / Playground / Changelog / Gallery の共通ヘッダー。各ページから `SiteHeader currentPath="..."` で利用。
- 検索・テーマ切替（light / dark / auto）・アクセシビリティメニュー（reduced motion / 高コントラスト / 文字サイズ）を持つ。テーマは `localStorage` キー `starlight-theme` を読み書きし、Starlight Docs の ThemeSelect と相互運用する。LP 系ページは `BaseLayout.astro` の inline script が初回描画前に `<html data-theme>` を初期化して FOUC を防ぐ。
- a11y 設定は `localStorage` (`tdsl-a11y-settings`) に保存し、`<html>` の `data-a11y-*` 属性として適用される。

### `DocsA11yMenu`

- Starlight の `components.SocialIcons` として登録し、Docs ヘッダーに a11y メニューを追加するコンポーネント（`site/src/components/DocsA11yMenu.astro`）。
- `SiteHeader.astro` と同一の `localStorage` キー・`data-a11y-*` 属性仕様を使うため、サイト全体で設定が共有される。テーマは Starlight 標準の ThemeSelect が `starlight-theme` / `data-theme` を管理し、LP 側トグル（#431）と自然に同期する。
...
- 左：コピー＋ CTA（primary: Playground / secondary: Docs）＋信頼バー（GitHub stars / License / Contributors / VS Code）＋ Latest release の 1 行。
- 右：エディタ風パネル（`.tdsl` ソース）＋レンダリング済みプレビュー＋ `tdsl check` 結果のコマンドストリップ。
- 「テキスト → 検証 → 描画」の 3 点が縦に並ぶことで、ツールのワークフローそのものをヒーローで示す。
- **実 WASM レンダリング（#428）**: preview は手書き HTML モックではなく、editor パネルと同じ `t("lp.hero.dsl_example")` を本体 WASM（`renderSvgFromSource`、`tdsl-wasm-node.mts` 経由）でビルド時にレンダリングした実 SVG（`class="tdsl-root"`）。editor と preview は単一ソースから導出されるため、DSL サンプルを変更すると preview も追従し、構文が壊れれば `astro build` が失敗して検知する。lane bridge（§8）により dark / high-contrast にも自動追従する。preview 全体は装飾扱い（`aria-hidden="true"`、情報は editor パネルとコピーで既に提供）のため、WASM が付与するツールチップ用 `tabindex="0"` はビルド時に除去し、aria-hidden 配下でのフォーカス可能要素混入（axe: aria-hidden-focus）を避ける。
- **CTA 階層**（#434）: hero の CTA は primary 1 ＋ secondary 1 に絞る。GitHub への導線はボタンではなく信頼バー（stars リンク）とヘッダーナビに集約し、focal point を 1 つに保つ。

### Closing CTA（#434）

- LP 本文の最後のセクション（install band の後・footer の前）。見出し + リード 1 文 + CTA（primary: Playground / secondary: Docs）のみで構成し、Calm density に従い 1 メッセージに絞る。
- install band は「導入コマンド」という手順情報に従事し、行動喚起（decision）はこのセクションが単一に担う。
- `.section-header` の既存 scroll reveal （`reveal-fade-up`）をそのまま利用し、新規の reveal ロジックは追加しない。
...
- **チャネル別タブ（#430）**: Homebrew / Shell（one-line installer）/ PowerShell / Cargo の 4 タブを WAI-ARIA Tabs パターン（`role="tablist"` / `role="tab"` / `role="tabpanel"`、矢印キー操作、automatic activation — 矢印キーで即座にパネルも切り替わる）で提供する。JS 無効時は全タブのパネルが `hidden` なしで縦積み表示される（プログレッシブエンハンスメント）。
- コマンド文字列は `site/src/data/install-channels.ts` を単一ソースとし、LP タブが参照する。Docs（`homebrew.mdx`）は Expressive Code の表示を保つため別実装のままだが、`install-channels.test.ts` が docs 本文に同一コマンド行が含まれることをテストで固定し、ドリフトを検知する。
- 各タブに Copy ボタン（`data-copy-target` で対応 ID を指定し、`navigator.clipboard.writeText` + `execCommand` フォールバック）を個別に持たせる。
- タブの下に「その他の導入方法（cargo / バイナリ手動など）を見る」で `/docs/homebrew/` への導線を残す。
...
`<html>` のデータ属性で 4 種類の調整を提供します（`SiteHeader` / `DocsA11yMenu` の a11y メニュー、または OS 設定が起点）。テーマ切替（#431）は別軸で、`<html data-theme="light|dark">` により light / dark を明示指定できる。`auto` は `localStorage["starlight-theme"]` を空文字列にし、初回描画前 script と `matchMedia("(prefers-color-scheme: light)")` で OS 設定へ解決する。
...
OS の `prefers-reduced-motion` / `prefers-contrast` も尊重しますが、ユーザーがメニューで明示的に切り替えた値が優先されます。テーマは `data-theme`（light / dark の明示指定）が OS の `prefers-color-scheme` より優先し、高コントラスト（`data-a11y-contrast="high"`）はその上に重なる。つまり優先順位は **theme explicit > OS color scheme**, **high contrast > base palette within the chosen theme**。
...
4. **`aria-hidden="true"`（`inert` 併用含む）でスクリーンリーダーからも隠されている装飾テキスト**: Hero の `.timeline-preview`（`980 - 1260` および WASM が生成する SVG 内のラベルテキスト。見出し文字列 `Rendered timeline` は #428 で `lp.hero.preview_header` として辞書化済み）、`.command-strip`、Feature card の `.feature-figure`（診断メッセージ・コマンド文字列・目盛りを含む）等。可視でも SR でも到達できないため翻訳の実益がない。
