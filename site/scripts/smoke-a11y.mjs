...
const baseUrl = normalizeBaseUrl(
  args.baseUrl ?? process.env.A11Y_BASE_URL ?? DEFAULT_BASE_URL
);
...
async function smokeA11y(rootUrl) {
  ...
  if (failures.length) {
    ...
    throw new Error(
      "a11y smoke failed: axe-core reported WCAG 2.1 AA violations."
    );
  }
  ...
  if (ALLOWED_EXCEPTIONS.length) {
    console.log(
      `a11y smoke: ${ALLOWED_EXCEPTIONS.length} documented exception(s) suppressed.`
    );
  }
}
...
async function smokeMobileNav(rootUrl) {
  ...
  try {
    ...
    if (!(await menuBtn.isVisible())) {
      throw new Error(
        "mobile nav: hamburger toggle should be visible at 375px viewport"
      );
    }
    ...
    if (navLinksDisplay !== "none") {
      throw new Error(
        "mobile nav: desktop .nav-links should be hidden at 375px viewport"
      );
    }
    ...
    if ((await menuBtn.getAttribute("aria-expanded")) !== "false") {
      throw new Error(
        "mobile nav: hamburger toggle should start with aria-expanded=false"
      );
    }
    ...
    if ((await menuBtn.getAttribute("aria-expanded")) !== "true") {
      throw new Error(
        "mobile nav: hamburger toggle should have aria-expanded=true after click"
      );
    }
    if (!(await dialog.isVisible())) {
      throw new Error(
        "mobile nav: dialog should be visible after hamburger click"
      );
    }
    ...
    for (let i = 0; i < navLinkCount; i += 1) {
      if (!(await navLinks.nth(i).isVisible())) {
        throw new Error(
          `mobile nav: nav link at index ${i} should be reachable without scrolling`
        );
      }
    }
    ...
    if (!focusInsideDialog) {
      throw new Error(
        "mobile nav: focus should be trapped inside the dialog when open"
      );
    }
    ...
    if ((await menuBtn.getAttribute("aria-expanded")) !== "false") {
      throw new Error(
        "mobile nav: Escape should close the dialog (aria-expanded=false)"
      );
    }
    ...
    if (!toggleHasFocus) {
      throw new Error(
        "mobile nav: focus should return to the hamburger toggle after Escape"
      );
    }
