...
## Lowering 警告（W208–W211）

lowering / validate 段階で発生する警告です。エラーではないためビルドは続行しますが、UTC オフセットの有無が食い違っていて前後関係を判定できないケースや、`map` / `apply` の解決結果に関わる問題を検知するために報告されます。`tdsl build` / `tdsl check` が `Warning:` として stderr に出力します。
...
**修正方法**: `tdsl lint --fix` では直せません（`fixable: false`）。順序が決まらないものを入れ替えても正しくはならないため、書き手が両側の表記を揃える必要があります。同じ状況を `tdsl check`（validate）は W208/W209 として報告します。
...
**原因**: `lane` ブロック（`group` 内も含む）で宣言された lane が、`span` / `event` / `template` / `apply` / `map` のいずれからも参照されていません。参照されない lane はレンダリング結果で空の帯になります。

**修正方法**: `tdsl lint --fix` では自動修正されません（これから item を追加する予定で先に宣言したケースを壊さないため）。不要であれば `lane` 宣言を削除し、必要であれば参照するアイテムを追加してください。
