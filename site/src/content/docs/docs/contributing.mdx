...
| `pnpm test:visual`      | LP（`/`）のスクリーンショット回帰（light / dark / high-contrast）。CI でも実行 |

UI を変更したときは、ブラウザ smoke（`pnpm smoke:playground:browser`）もローカルで一度通してください（CI では走りません）。初回は `pnpm exec playwright install chromium` が必要です（`test:visual` にも同じ Chromium が必要です）。

LP のレイアウト・配色・タイポグラフィを変更した場合は `pnpm test:visual` を実行し、差分が意図した変更であることを確認してから `pnpm test:visual:update` でベースラインを更新してください。ベースライン画像の変更は目視レビューが必要な差分として PR に含めます（このコマンドは CI からは実行しません）。
