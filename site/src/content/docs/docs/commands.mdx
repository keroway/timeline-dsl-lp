...
Timeline DSL の言語サーバ（LSP）を stdio 経由で起動します（v1.11.0〜）。エディタの LSP クライアントから接続すると、`.tdsl` 編集中にリアルタイム診断・補完・ホバー・定義ジャンプが利用できます。

```sh
tdsl lsp
```

`tdsl lsp` は引数を取らず、標準入出力で LSP の JSON-RPC を話します。エディタ側の設定や対応機能の一覧は [インストール](/docs/installation/#言語サーバ-lsp) を参照してください。
