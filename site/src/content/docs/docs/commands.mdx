...
`.tdsl` をスタンドアロンHTML / SVG / PNG として描画します。`--format` で出力形式を切り替えます。`--format png` は v1.10.0〜対応です。
...
PNG 出力は内部 SVG を `resvg` でラスタライズします。CJK レーンラベルは system fonts（Noto Sans JP / Hiragino Sans / Yu Gothic 等）が読み込まれている環境で描画できます。
