...
`import-csv` は `lane,type,start,end,time,label,tags,id` のヘッダー行を持つUTF-8 CSVを入力にします。`start` / `end` / `time` 列は `YYYY` / `YYYY-MM` / `YYYY-MM-DD` の 3 精度を受理します（月・日精度は v1.10.0〜）。任意で `source` / `origin` 列も受理し、`export-csv` が出力する10列のCSVをそのまま往復できます（v1.26.0〜）。

### export-csv

IR（`.tdsl` または `.json`）をCSVへ書き出します（v1.23.0〜）。`import-csv` が受理する8列（`lane,type,start,end,time,label,tags,id`）に `source` / `origin` を加えた10列を出力し、`source` / `origin` を含めて全10列が `import-csv` とラウンドトリップします（v1.26.0〜）。`origin=wikidata` の行は `source` が `wd:Q<id>` 形式であることが必須で、不整合はCSV行番号付きエラーになります。
