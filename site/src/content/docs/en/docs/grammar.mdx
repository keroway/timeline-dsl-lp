...
:::caution
As of this writing, validating a `.tdsl` file containing `now` in the browser Playground crashes the WASM module (`SystemTime::now()` is not implemented for the `wasm32` target). The CLI (`tdsl check` / `tdsl build` / `tdsl render`) is unaffected. Until this is fixed upstream, verify `now` usage with the CLI rather than the Playground.
:::
