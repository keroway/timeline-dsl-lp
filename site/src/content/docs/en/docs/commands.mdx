...
Formats `.tdsl` to the canonical style (2-space indentation, one blank line between blocks) (v1.14.0+). By default it writes the formatted result to standard output. Use `--write` to overwrite the file in place, or `--check` to exit non-zero when there is a diff (for CI). Also accepts multiple files and directories (see [CLI Reference](/en/docs/cli-reference/#fmt) for details, v2.0.0+).
