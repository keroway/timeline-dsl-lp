...
Renders `.tdsl` to a standalone HTML, SVG, or PNG file. Use `--format` to choose the output format. `--format png` is supported from v1.10.0+.
...
PNG output rasterizes the internal SVG with `resvg`. CJK lane labels are rendered as long as the host environment has system fonts available (Noto Sans JP, Hiragino Sans, Yu Gothic, etc.).
...
Starts the Timeline DSL language server (LSP) over stdio (v1.11.0+). Once connected from your editor's LSP client, you get real-time diagnostics, completion, hover, and goto-definition while editing `.tdsl` files.

```sh
tdsl lsp
```

`tdsl lsp` takes no arguments and speaks the LSP JSON-RPC protocol over standard input/output. See [Installation](/en/docs/installation/#language-server-lsp) for editor configuration and the list of supported features.
