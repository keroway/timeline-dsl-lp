...
`tdsl` handles validation, compilation to JSON IR, HTML rendering, Wikidata-based timeline generation, and template scaffolding for `.tdsl` files. See the [CLI Reference](/en/docs/cli-reference/) for the detailed arguments and options of each subcommand.
...
`import-csv` takes a UTF-8 CSV with a header row of `lane,type,start,end,time,label,tags,id`. The `start` / `end` / `time` columns accept three precisions — `YYYY`, `YYYY-MM`, and `YYYY-MM-DD` (month/day precision is supported from v1.10.0+). It also optionally accepts `source` / `origin` columns, so it round-trips the 10-column CSV produced by `export-csv` as-is (v1.26.0+).

### export-csv

Exports an IR (from `.tdsl` or `.json`) to CSV (v1.23.0+). Writes the 8 columns accepted by `import-csv` (`lane,type,start,end,time,label,tags,id`) plus `source` / `origin`, for a total of 10 columns. All 10 columns, including `source` / `origin`, round-trip with `import-csv` (v1.26.0+). Rows with `origin=wikidata` require `source` to be in the `wd:Q<id>` form; a mismatch produces a CSV row-numbered error.
