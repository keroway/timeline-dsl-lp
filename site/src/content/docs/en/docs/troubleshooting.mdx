...
See the [Error Code Reference](/en/docs/errors/) for the meaning of diagnostic codes such as `E001` and how to fix them.
...
## Errors/warnings from `--chart-pagination`

`--chart-pagination <N>` (added in v1.28.0; see [CLI Reference](/en/docs/cli-reference/#chart-pagination)) fails under these conditions:

- `--chart-pagination is not supported with --watch` — combined with `--watch`
- `--chart-pagination must be >= 1` — `N` is `0`
- `--chart-pagination only supports --format svg or --format pdf` — combined with `--format html` / `png`
- `--chart-pagination requires --output <file> (stdout cannot hold multiple pages)` — `--output` is missing (stdout cannot represent multiple files or pages)
- `--pdf-pagination requires --show-table (nothing to paginate otherwise)` — `--pdf-pagination` is specified without `--show-table`

Rather than an error, if a lane `group` is split across a page boundary, this warning is printed to stderr (the output is still generated and the command still succeeds):
...
Increase the `--chart-pagination` value, or reorder lanes (`order`), so that lanes sharing a `group` stay on the same page.

`--chart-pagination` is CLI-only — the Playground (WASM in the browser) does not expose the pagination logic, so it cannot be reproduced there. Use the [CLI](/en/docs/installation/) (`tdsl render`) to try it.
