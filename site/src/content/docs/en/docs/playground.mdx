...
<LinkButton href="/playground/" target="_blank" variant="primary">Open Playground</LinkButton>
...
If parsing fails, only an error toast is shown and the editor is left unchanged. Comments are not preserved by the AST round-trip, so they are removed by formatting (a warning is shown when the source contains comments).
