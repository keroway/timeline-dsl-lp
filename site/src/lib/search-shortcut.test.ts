...
describe("isSearchShortcut", () => {
  ...
  it("IME 変換中 (isComposing) なら false", () => {
    expect(isSearchShortcut(keyEvent({ ctrlKey: true, key: "k", isComposing: true }))).toBe(false);
  });

  it("キーリピート中 (repeat) なら false", () => {
    expect(isSearchShortcut(keyEvent({ ctrlKey: true, key: "k", repeat: true }))).toBe(false);
  });
...
describe("getSearchShortcutHint", () => {
  it("Mac の userAgent なら ⌘K", () => {
    expect(getSearchShortcutHint("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)")).toBe("⌘K");
  });

  it("iPhone/iPad の userAgent なら ⌘K", () => {
    expect(getSearchShortcutHint("Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X)")).toBe(
      "⌘K",
    );
  });

  it("Windows の userAgent なら Ctrl+K", () => {
    expect(getSearchShortcutHint("Mozilla/5.0 (Windows NT 10.0; Win64; x64)")).toBe("Ctrl+K");
  });

  it("Linux の userAgent なら Ctrl+K", () => {
    expect(getSearchShortcutHint("Mozilla/5.0 (X11; Linux x86_64)")).toBe("Ctrl+K");
  });
});
